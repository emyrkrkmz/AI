...
#plot_importance(rt_final, X) # num for # of feature will shown
...
#val_curve_params(rt_final, X, y, "max_depth", range(1,11), scoring="roc_auc")
...
lgbm_params = {"learning_rate": [0.01, 0.02, 0.1],
               "n_estimators": [100, 250, 300, 350, 500,],
               "colsample_bytree": [0.5, 0.7, 1]}
...
catboost_model = CatBoostClassifier(random_state=17, cerbose=False)
...
catboost_best_grid = GridSearchCV(catboost_model, cat_params, cv=5, n_jobs=-1, verbose=True).fit(X,y)
...
rf_random_params = {"max_depth": np.random.randint(5, 50, 10),
                    ...
                    "n_estimators": [int(x) for x in np.linespace(start=200, stop=1500, num=10)]}


rf_random = RandomizedSearchCV(estimator=rf_model,
                               param_distributions=rf_random_params,
                               n_iter=100, # # of parameter to try
                               verbose=True,
                               random_state=42,
                               n_jobs=-1)
